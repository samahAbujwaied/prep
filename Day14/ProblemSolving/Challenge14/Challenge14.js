...
const replaceWords = (str) => {
    ...
    let I= splits.indexOf('I');
    let am = splits.indexOf('am');
    let was = splits.indexOf('was');
    if (I>=0){ splits.splice(I,1,'We'); }
    if(am>=0){splits.splice(am,1,'are');}
     if (was>=0){splits.splice(was,1,'were'||" ");}
     return splits.join(" ")
}
...
const letterCounter = (str) => {
    ...
    data.forEach(element => {
        ...
        for (let i = 0 ; i < element.length-1; i++){
            if(element[i]==element[i+1])counter++; 
            else {word += `${element[i]}${counter}`;
              counter = 1;}
        }
        word += `${element[element.length-1]}${counter}`
        result.push(word);
        counter = 1;   
    });
    return result.join(' ')
